...
- 강조할 내용에 포인트 표시 할 것.
- 키워드를 자세히 설명할 수 있도록 스크립트를 작성할 것.

### 프로젝트 - PPT 내용 리뷰 II 및 시연 리뷰(2018-12-14)

- PPT 검토
    - 유사 서비스 장단점에 비교 페이지에 벤치마킹 제목을 달 것.
    - PT 페이지를 발표에 맞춰 여러 페이지로 쪼갤 것.
    - PT 페이지를 설명할 때 중간에 키워드를 출력하여 청중의 집중을 유도할 것.
    - 산출물 소개할 때 설명 중에 일부 화면을 잘라서 팝업으로 보여 줄 것.
    - 버스킹 위치는 검색하는 상황으로 합칠 것.
- 발표스크립트 검토 
    - 시연에서 관리자의 경우 사용자의 입장이 아닌 설명자의 입장으로 말해도 됨.
- 프로그램 시연 검토
    - 이전 발표 팀에서 로그인 설명했으면 생략해도 됨.
    - 무대 제공처의 지도에서 버스 정류장 마커 제거할 것. 위치 마커와 헷갈림.
    - 파일 업로드 후 동영상 출력이 지연되기 때문에 동영상에서 지연되는 화면 자를 것.
    - 버스킹 상세 정보에서 정보가 하나로 묶일 수 있도록 가로 선을 제거할 것.
